...
\begin{itemize}
	...
	Эта команда проверяет установлен ли пакет в системе. \Sys{rpm -q} означает <<query>>.
	...
	(запросить информацию). Но в случае \Sys{rpm -i <имя пакета>}, \Sys{-i} означает инструкцию \Sys{install} (установи).

	\item \textbf{Просмотр установленных пакетов}: \Sys{rpm -qa}\\
	Эта команда выводит список всех установленных пакетов в системе. \Sys{-a: all} (весь, все). \Sys{rpm -qa} означает <<query all>>.
	...
	(нуждается). Например, \Sys{rpm -qR} означает <<query requires>> (запрос небоходимого, запрос зависимостей).
	...
	(файл). Например, \Sys{rpm -qf} означает <<query file>> (запрос файла).
	...
	(список). Например, \Sys{rpm -ql} означает <<query list>> (запрос списка).
	...
	(инсталировать, установить). Например, \Sys{rpm -i package.rpm} установит содержимое пакета в системе.
	...
	Команда \Sys{rpm -U} обновляет пакет до новой версии или установит пакет, \Sys{rpm -F}
	только обновит пакет, если он уже установлен. \Sys{-U: Update}(обновить), \Sys{-F: Freshen}(освежить).
	...
	\begin{itemize}
	 ...
	 (инсталяция подробно) и используется для вывода более подробной информации в процессе установки пакета. Подробный вывод существует не для всех ключей утилиты.
		\item \textbf{\Sys{----quiet}}: (тихо). Вывести минимальный набор сообщений если возможно. Например, \Sys{rpm -i ----quiet} означает <<install quiet>> (инсталяция тихая).
	\end{itemize}
...
\marginalia{ex_sign_col}{Для пакетов установленных в системе можно указывается только имя, если установлена одна версия пакета, и Sys{имя-версию-релиз}, если
установлено больше одной версий.}
