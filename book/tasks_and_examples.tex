...
Расположим архив и \Sys{.spec}-файл в соответствии правилами структуры каталогов
\Sys{RPM}:
\begin{verbatim}
...
В ходе выполнения команда скомпилирует исходные данные и соберет
...
метаданных. Посмотрим информацию о пакет, список файлов пакета и удалим
пакет из системы:
\begin{verbatim}
...
переданым специально подготовленным архивам \Sys{pkg.tar} и пакетам
...
Создадим изолироване сборочное окружение \Sys{Hasher}:
...
Войдя в окружение \Sys{Hasher} мы окажемся в каталоге \Sys{/.in}.
...
Выполним сборку с использованием \Sys{gear-rpm} и передадим ключ \Sys{-bb}
...
\Sys{--verbose -- --verbose} для подробного вывода информации процесса выполнения
...
изменения с применением механизма \Sys{GEAR} авто генерации патча.
...
Удобным для Вас текстовым редактором измените в файле \Sys{HelloUniverse/HelloUniverse.cpp}
...
Нас интересует метка \Sys{v3.0}. Притянем изменения в рабочее файловое дерево:
\begin{verbatim}
git merge v3.0
\end{verbatim}
...
\Sys{git}-репозиторием. Формирование репозитория в таком случае остается на
