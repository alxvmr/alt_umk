...
\begin{itemize}
	...
	\item \Emph{gear} --- пакет содержит утилиты для сборки пакетов \Sys{rpm} из \Sys{gear}-репозитория и управления \Sys{gear}-репозиториями;
...
\marginalia{ex_sign_col}{Методическое пособие расчитано на работу с файлом  \Sys{$\sim$/.rpmmacros}
вида:
\\\\
...
\\\\
где имя, фафмилия и почтовый адрес соответствуют используемым данным пользователя.
...
\begin{itemize}
	...
	\begin{itemize}
		\item Макрос \Sys{\%setup} перемещает файлы и распаковывает архивы с исходными данными в католог для сборки,
...
\item Понятие \Sys{buildroot} в пространстве сборки RPM-пакетов, какое назначение?
