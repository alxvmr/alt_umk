...
\begin{itemize}
	...
	\item \Sys{RPM} позволяет брать оригинальные источники обеспечения и упаковывать их в
		пакеты с исходными данными (\Sys{.src.rpm}) и а бинарные пакеты (\Sys{.rpm}). В пакетах \Sys{.src.rpm}
...
\begin{itemize}
	...
	\item \Emph{Gear} --- набор инструментов для упрощения процесса сборки  \Sys{RPM}-пакетов. \Sys{Gear}
...
\begin{itemize}
	...
	\item \Emph{gear} --- этот пакет содержит утилиты для сборки пакетов \Sys{rpm} из \Sys{gear}-репозитория и управления \Sys{gear}-репозиториями;
