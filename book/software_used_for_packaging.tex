...
Использование специализированных пакетов, в отличии от обычного файлового архива, имеет ряд
...
\begin{itemize}
	...
	\item \Sys{RPM} позволяет брать оригинальные источники обеспечения и упаковывать их в
		пакеты с исходными днными (\Sys{.src.rpm}) и бинарные пакеты (\Sys{.rpm}). В пакетах \Sys{.src.rpm}
...
\begin{itemize}
	\item \Emph{RPM}  В контексте данной темы \Sys{RPM} рассматривается не только как менеджер пакетов,
		но и как набор инструментов для их сборки (\Sys{rpmbuild}). Отличительными особенностями \Sys{RPM}
	...
	\item \Emph{Hasher} --- инструмент позволяет производить сбоку \Sys{RPM}-пакетов в изолированой среде,
		что обеспечивает повышенную безопастность  и идентичный результат независимый от стороннего
		программного обеспечения установленного в системе.
	\item \Emph{Gear} --- набор инструментов для упрощения процеса сборки  \Sys{RPM}-пакетов. \Sys{Gear}
		интегрирован с системой контроля версий \Sys{git},  \Sys{hasher} и \Sys{rpmbuild}.
		Позволяет подготавлить \Sys{SRPM} из \Sys{git} репозитория, или использовать уже существующий
		\Sys{SRPM}. Подготавливает исходные данные пакета и запускает \Sys{hasher} или \Sys{rpmbuild}.
\end{itemize}

\section{Программное обеспечение для сборки RPM-пакетов}

Перечислим набор основных-базовых пакетов и входящих в их состав программ необходимых для
сборки rpm-пакета%
...
\begin{itemize}
	...
	\item \Sys{rpm-build} --- пакет содержит сценарии и исполняемые программы, которые используются для сборки пакетов с помощью \Sys{RPM};
	\begin{itemize}
...
\begin{itemize}
	\item \Emph{RPM-пакет} (бинарные) --- это архив с расширением \Sys{.rpm}. Такой пакет готов к установке в
...
Файлы бинарного пакета могут быть скомпилированные под определённую процессорную архитектуру(аппаратную платформу).
...
в своем имени маркировку архитектуры \Emph{noarch} --- платформо-независимый бинарный пакет
имеет вид: \Sys{*.\{noarch\}.rpm}. Например упакованные картинки рабочего стола или наборы иконок.
...
\marginalia{ex_sign_col}{Например для процессоров ARM пакет будет выглядеть так: \Sys{admc-0.16.0-alt1.armh.rpm}. Для
архитектурно-независимого пакета могут выглядеть так \Sys{icon-theme-simple-sl-2.7-alt4.noarch.rpm}.}

Имя установленного пакета в системе может отличаться от имени пакета: в командной строке можно обратиться за информацией
...
\Emph{Рабочаее пространство сборки \Sys{RPM}-пакета} --- это структура файлов и каталогов. Эту структуру
...
Секция журнал изменений содержит форматированый в принятом сообществом формаие журнал
с описанием изменений проводимых с исходными данными и сборочными инструкциями пакета.

\marginalia{ex_sign_col}{В пакете rpm-utils присутсвует утилита \Sys{add\_changelog}. Утилита добавит
заголовок для журнала изменений на основе данных в преамбуле spec  файла.}
